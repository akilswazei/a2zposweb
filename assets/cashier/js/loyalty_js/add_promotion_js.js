...
$('#promotion_apply_type').on('change', function() {
    if ($(this).val() == '1') {
      $('#autoapply').val(0);
...
$('#promotion_name').on('blur', function() {
    ...
    $.ajax({
        url: base_url + "cashier/Cashier/checkPromotion",
        type:'post',
...
$('#btnSave').on('click', function() {
  ...
  if(coupon_state == true) {
      $.ajax({
        type: 'ajax',
        method: 'post',
        url: base_url + "cashier/add_promotion",
        data: $('.add-promotion').serialize(),
        dataType: 'json',
        beforeSend: function(){
