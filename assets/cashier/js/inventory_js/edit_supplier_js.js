...
$('#suppName').on('blur', function() {
    var supplier_name = $('#suppName').val();
    var supplier_id = $('#supplier_id').val();

    $.ajax({
        url: base_url + "cashier/Cashier/checkSupplierExist",
        type: 'post',
        data: {supplier_name: supplier_name, supplier_id:supplier_id},
        success: function(response) {
            // console.log(response);
            if (response == 'fail') {
               $('#suppName').addClass('supperr');
               name_state = false;
               $("#fname_err").html("Supplier name already exists").show();
            } else if (response == 'success') {
                name_state = true;
                $('#suppName').removeClass('supperr');
                $("#fname_err").html("").show();
            }
        }
    });
 ...
 $('#btnSave').click(function(){
    ...
    if($('#suppName').hasClass('supperr') == false ){
      var formdata = $('.edit-supplier')[0];
      $.ajax({
          ...
          success: function(data){
                if(data == 'success'){
                  swal({
                    title: "Success!",
                    text: "Supplier is updated successfully!",
                    icon: "success",
                    buttons: false,
                  });
                  setTimeout( function (){
