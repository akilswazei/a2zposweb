        ...
        <div class="row">
            <div class="col-md-12 p-0">
                ...
                <nav class="navbar navbar-expand-lg navbar-light bg-light navbar-dark bg-rep bg-dark py-3 px-4">
                    ...
                    </button> <span class="navbar-brand">Sales And Margin Comparison</span>
            ...
            <div class="row px-12">
                ...
                <div class="col-md-3">
                    <div class="form-group">
                        ...
                        <select class="form-control" aria-label="Default select example" name="top_selling_cats_filter" id="top_selling_cats_filter">
                            <option value="all" <?php if(count($category_id_arr_return) == 0) {echo 'SELECTED';} ?>>Select All Category</option>
                    ...
                    <table class="table table-sm cell-border" id="tbl_inventory" style="width:100%">
                        <thead class="headsec">
                            <tr>
                                ...
                                <th class="text-white">Margin</th>
                                <th class="text-white">Margin(%)</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php
                                if(!empty($sm_reports_sales_margin)) {
                                    foreach ($sm_reports_sales_margin as $key => $value) {
                            ?>
                            <tr>
                                <td><?php print !empty($value->product_name) ? $value->product_name : "-"; ?></td>
                                <td><?php print !empty($value->category_name) ? $value->category_name : "-"; ?></td>
                                 <td><?php print date("m/d/Y", strtotime($value->order_date)); ?></td>
                                <td><?php print ($value->sale_price_final * $value->qty); ?></td>
                                <td><?php print ($value->supplier_price * $value->qty); ?> [<?php print $value->supplier_price; ?> x <?php print $value->qty; ?>]</td>
                                <td><?php print ($value->margin * $value->qty); ?></td>
                                <td>
                                    <?php
                                    if(!empty($value->margin) && !empty($value->sale_price_final)) {
                                        print number_format(($value->margin * 100) / $value->sale_price_final,2)."%";
                                    } else {
                                        print "0";
                                    }
                                    ?>                                        
...
<script type="text/javascript">
    $(document).ready(function() {
        $(document).on("click","#frmDetailsFilterBtn",function() {
            ...
            var top_selling_cats_filter  = $("#top_selling_cats_filter").val();
            window.open('<?php print base_url(); ?>cashier/sm_reports_details/'+top_selling_cats_filter+'/<?php print $day_of_date_return; ?>'+'/'+start_date_filter+'/'+end_date_filter, '_self').focus();
        });

        $(document).on("click","#frmClearFilterBtn",function() {
            $("#overlay,.loader").show();
            var top_selling_cats_filter = "all";
            var start_date_filter = "";
            var end_date_filter = "";
            window.open('<?php print base_url(); ?>cashier/sm_reports_details/'+top_selling_cats_filter+'/<?php print $day_of_date_return; ?>'+'/'+start_date_filter+'/'+end_date_filter, '_self').focus();
        });

        var tbl_inventory_sm = $('#tbl_inventory').DataTable({
            "pageLength": 10,
            "autoWidth": true,
            "bLengthChange": false,
            "responsive": true,
            ...
            buttons: [
                {
                    ...
                    text: 'Email',
                    action: function () {
                        extendDataTable(tbl_inventory_sm,"Sales And Margin Comparison");
                    }
                }
            ],
        });
