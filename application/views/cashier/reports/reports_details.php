        ...
        <div class="row">
            <div class="col-md-12 p-0">
                <nav class="navbar navbar-expand-lg navbar-light bg-light navbar-dark bg-rep bg-dark py-3 px-4">
                    ...
                    </button> <a class="navbar-brand" href="javascript:;" onclick="javascript: history.back(-1);">Top <?php if(!empty($no_of_items_filter_return)) print $no_of_items_filter_return; else print "10"; ?> Selling Items</a>
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            <label>No. of Items</label>
                                <input type="text" name="no_of_items_filter" id="no_of_items_filter" class="form-control" value="<?php if(!empty($no_of_items_filter_return)) print $no_of_items_filter_return; else print "10"; ?>">
                        </div>
                    </div>
                ...
                <div class="col-md-12">
                    <div class="">
                        ...
                        <table class="table table-sm cell-border" id="tbl_inventory" style="width:100%">
                            ...
                            <tbody>
                                <?php
                                    if(!empty($reports_details)) {
                                        foreach ($reports_details as $key => $value_tsi) {
                                ?>
                                <tr>
                                    <td><?php print $value_tsi["product_name"]; ?></td>
                                    <td><?php print $value_tsi["category_name"]; ?></td>
                                    <td><?php if($value_tsi["coupon_name"] != "") print $value_tsi["coupon_name"]; else print "-"; ?></td>
                                    <td><?php print date("m/d/Y", strtotime($value_tsi["order_date"])); ?></td>
...
<script type="text/javascript">
    $(document).ready(function() {
        $(document).on("click","#frmDetailsFilterBtn",function() {
            ...
            if($("#product_name_filter").val() != "") {
                var product_name_temp_btoa = window.btoa($.trim($("#product_name_filter").val()));
                product_name_temp_btoa = product_name_temp_btoa.replaceAll('=','_');
                product_name           = encodeURIComponent(product_name_temp_btoa);
            }

            window.open('<?php print base_url(); ?>cashier/reports_details/'+category_id_encoded+'/'+no_of_items_filter+'/'+product_name+'/'+start_date_filter+'/'+end_date_filter, '_self').focus();
        });

        $(document).on("click","#frmClearFilterBtn",function() {
            $("#overlay,.loader").show();
            var category_id_encoded = 0;
            var no_of_items_filter  = 10;
            var product_name = "none";
            var start_date_filter = "";
            var end_date_filter = "";
            window.open('<?php print base_url(); ?>cashier/reports_details/'+category_id_encoded+'/'+no_of_items_filter+'/'+product_name+'/'+start_date_filter+'/'+end_date_filter, '_self').focus();
            ...
            buttons: [
                {
                    ...
                    text: 'Email',
                    action: function () {
                        extendDataTable(tbl_inventory,"Top Selling Items");
                    }
                }
            ],
        });
