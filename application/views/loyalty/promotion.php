                        ...
                        <div class="customcard">
                            <div>
                                <div class="usercardheader">
                                    ...
                                    <a href="<?=base_url('Loyalty/promotion')?>" type="button" class="btn btn-outline-dark alluserbtn adduserbtn">Add Promotion   
                                ...
                                <div class="customcardbody2 ">
                                    <div class="table-responsive">
                                        <table class="table table-striped table-sm" id="tbl_promotion">
                                            <thead>
                                                <tr>
                                                    <th>#</th>
                                            ...
                                            <tbody>
                                                <?php $i=1; if(!empty($promotion)){ foreach ($promotion as $p) { ?>
                                                <tr>
                                                    <td><?=$i?></td>
                                                    <td><?=$p['coupon_name']?></td>
                                                    ...
                                                    <td>
                                                        <?php if($p['status'] == 1){?>
                                                                <button data-id="<?php echo $p['coupon_id'];?>" data-status='0' class="btn btn-rounded deactivate-promotion" data-toggle="tooltip" title="Deactive"><i class="fa fa-close"></i></button>
                                                        <?php }else{?>
                                                            <button data-id="<?php echo $p['coupon_id'];?>" data-status='1' class="btn btn-rounded deactivate-promotion" data-toggle="tooltip" title="Active"><i class="fa fa-check"></i></button>
                                                        <?php }?>     
                                                    </td>
                                                    <td style="text-align: center;">
                                                        <a href="<?=base_url('Loyalty/edit_promotion?id='.$p['coupon_id'])?>" type="button" class="btn btn-outline-dark alluserbtn">Edit 
                                                        ...
                                                        <button type="button" data-id="<?php echo $p['coupon_id'];?>" class="btn btn-outline-dark alluserbtn deleteRecord">Delete
     ...
     <div class="modal fade" id="deleteModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
      <div class="modal-dialog modal-dialog-centered " role="document">
        <div class="modal-content">
         <form class="delete-Role" method="post" action="<?php echo base_url('Loyalty/delete_promotion'); ?>">
                ...
                <div class="row">
                  <div class="col-md-12 mt-2 mb-3 ">
                   ...
                   <h5 class="text-center">Are you sure to delete this record ?</h5>
                      ...
                      <input type="hidden" name="promotion_id" id="delete_id" class="form-control">
                </div>
              </div>
            </div>
...
<script type="text/javascript">
    $('#tbl_promotion').on('click', '.deleteRecord', function() {
        var delete_id = $(this).data('id');
        $('#delete_id').val(delete_id);
        $('#deleteModal').modal('show');
    });

    $('#tbl_promotion').on('click', '.deactivate-promotion', function() {
        ...
        $.ajax({
            type: 'ajax',
            method: 'post',
            url: '<?=base_url("Loyalty/change_promotion_status")?>',
