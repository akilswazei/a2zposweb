                        ...
                        <div class="customcard">
                            <div>
                                <div class="usercardheader">
                                  ...
                                  <!--   <a href="<?=base_url('LoyaltyPoint')?>" type="button" class="btn btn-outline-dark alluserbtn adduserbtn">Add a loyalty Point   
                                ...
                                <div class="customcardbody2 ">
                                    <div class="table-responsive">
                                        <table class="table table-striped table-sm" id="tbl_point">
                                            <thead>
                                                <tr>
                                                    <th>#</th>
                                                    <th>Point Type</th>
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    <td><?=$i?></td>
                                                    <td><?php if($p['point_type']==1)echo 'New Register';
                                                                else
                                                                echo 'By Value';    ?></td>
                                                    <td><?=$p['point_amount']?></td>
                                                    <td><?=$p['point']?></td>
                                                    <td><?=date('d-m-Y',strtotime($p['created_date']))?></td>
                                                  
                                                    <td>
                                                        <?php if($c['status'] == 1){?>
                                                                <button data-id="<?php echo $p['point_id'];?>" data-status='0' class="btn btn-rounded deactivate-coupon" data-toggle="tooltip" title="Deactive"><i class="fa fa-close"></i></button>
                                                            <?php }else{?>
                                                                <button data-id="<?php echo $c['point_id'];?>" data-status='1' class="btn btn-rounded deactivate-coupon" data-toggle="tooltip" title="Active"><i class="fa fa-check"></i></button>
                                                            <?php }?>     
                                                    </td>
                                                    <td style="text-align: center;">
                                                        
                                                            
                                                        <a href="<?=base_url('LoyaltyPoint/edit_point?id='.$p['point_id'])?>" type="button" class="btn btn-outline-dark alluserbtn">Edit 
     ...
     <div class="modal fade" id="deleteModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
      <div class="modal-dialog modal-dialog-centered " role="document">
        <div class="modal-content">
         <form class="delete-Role" method="post" action="<?php echo base_url('LoyaltyPoint/delete_point'); ?>">
                ...
                <div class="row">
                  <div class="col-md-12 mt-2 mb-3 ">
                   ...
                   <h5 class="text-center">Are you sure to delete this record ?</h5>
                      ...
                      <input type="hidden" name="point_id" id="delete_id" class="form-control">
                </div>
              </div>
            </div>
...
<script type="text/javascript">
    $('#tbl_point').on('click', '.deleteRecord', function() {
        var delete_id = $(this).data('id');
        $('#delete_id').val(delete_id);
        $('#deleteModal').modal('show');
    });

    $('#tbl_point').on('click', '.deactivate-coupon', function() {
        ...
        $.ajax({
            type: 'ajax',
            method: 'post',
            url: '<?=base_url("Loyalty/change_status_point")?>',
